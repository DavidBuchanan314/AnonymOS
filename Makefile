...
KERNEL.BIN: kernel.c rootfs
	i686-elf-gcc -o rootfs/KERNEL.BIN kernel/kernel.c -ffreestanding -Ofast -nostdlib -Wl,--oformat=binary,-Ttext=0x10000 -Wa,--32 -Wall -Wextra -Wpedantic

rootfs:
	mkdir -p rootfs

test: boot.bin
	qemu-system-i386 -enable-kvm -m 512M -drive format=raw,file=boot.bin -serial stdio 2>/dev/null
